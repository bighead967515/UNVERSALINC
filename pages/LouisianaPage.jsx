  ...
  return (
    <motion.div variants={cardVariants} className="bg-[#262626] rounded-2xl overflow-hidden shadow-lg hover-glow flex flex-col">
      <img  
       ...
       src="https://images.unsplash.com/photo-1560972343-6344029c1efe" />
...
const LouisianaPage = () => {
  ...
  useEffect(() => {
    const fetchShops = async () => {
      ...
      const { data, error } = await supabase
        .from('artists')
        .select('*')
        .or('location.ilike.%Louisiana%,location.ilike.%New Orleans%,location.ilike.%Baton Rouge%,location.ilike.%Lafayette%,location.ilike.%Ponchatoula%,location.ilike.%Lake Charles%,location.ilike.%Denham Springs%,location.ilike.%Larose%')
        .order('rating', { ascending: false });

      if (error) {
        console.error("Error fetching LA shops:", error);
        setError("Could not fetch the best shops in Louisiana. Please try again later.");
      } else {
        setShops(data);
      }
      setLoading(false);
  ...
  return (
    <>
      <Helmet>
        <title>Best Tattoo Shops in Louisiana | InkConnect</title>
        <meta name="description" content="Discover the top-rated tattoo shops across Louisiana, from New Orleans to Baton Rouge and Lafayette." />
      </Helmet>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <div className="container mx-auto px-4">
          ...
          >
            <h1 className="text-4xl lg:text-6xl font-bold mb-4">
              The Best of <span className="gradient-text">Louisiana</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              A curated list of the most iconic and highly-rated tattoo shops across the Pelican State.
            </p>
          </motion.div>
          ...
          {loading ? (
            <div className="flex flex-col justify-center items-center min-h-[50vh]">
              ...
              <p className="text-lg text-muted-foreground mt-4">Exploring the Bayou for Artists...</p>
            </div>
          ) : error ? (
